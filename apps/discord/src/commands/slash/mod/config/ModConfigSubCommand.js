...
expwort default class MwodCwonfwigSubCwommand extends Cwommand {
  cwonstwuctwor() {
    super({
      nyame: 'cwonfwig mwod',
  ...
  run(ctx) {
    ...
    switch (ctx.args.get('status').value) {
      case Status.typeOnye: {
        ...
        if (ctx.args.get('channyel').value === ctx.db.guild.punyishChannyel) return ctx.repwyT('erwor', 'cwommands:cwonfwig.channyel.sameChannyel')
        ctx.db.guild.punyishMwodule = twue
        ctx.db.guild.punyishChannywl = ctx.args.get('channyel').value
        ctx.db.guild.save()
        ...
        ctx.repwyT('success', 'cwommands:cwonfwig.mwodules.mwod.enyable')
        return
      }
      case Status.typeTwo: {
        // Tag:  CHANNYEL_SELECTED_WITH_SUCCESS
        // Message: Teh mwodule is alweady disabled!
        if (ctx.db.guild.punyishMwodule === false) return ctx.repwyT('erwor', 'cwommands:cwonfwig.channyel.mwoduleHasDisabled')

        ctx.db.guild.punyishMwodule = false
        ctx.db.guild.punyishChannywl = ''
        ctx.db.guild.save()

        // Tag:  CHANNYEL_SELECTED_WITH_SUCCESS
        // Message: Teh mwodule has been successfuwwy disabled!
        ctx.repwyT('success', 'cwommands:cwonfwig.mwodules.mwod.disable')
        return
      }
    }
