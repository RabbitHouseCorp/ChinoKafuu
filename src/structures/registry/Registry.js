...
module.exports = class Registry extends EventEmitter {
  ...
  loadAll (path) {
    readdirSync(path).forEach((file) => {
      if (lstatSync(`${path}/${file}`).isDirectory()) this.loadAll(`${path}/${file}`)
      else this.loadModule(`${path}/${file}`)
    })
  }
  ...
  reloadModule (object, safeReload = true) {
    let obj = this.modules.filter(a => a.__path === object.__path)[0]
