...
module.exports = class LavalinkPlayer extends EventEmitter {
    ...
    async getSongs(node, search) {
        ...
        return axios.get(`http://${node.host}:${node.port}/loadtracks?${params.toString()}`, { headers: { Authorization: node.password } }).then(res => res.data.tracks).catch(err => {
    ...
    playAnimu() {
        return this.getSongs(this.player.node, 'https://cast.animu.com.br:8021/stream').then(result => { 
            if (!result[0]) return
            this._addToQueue(result[0])
            return result[0].info
        })
