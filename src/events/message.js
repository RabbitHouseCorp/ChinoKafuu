...
module.exports = class MessageReceive {
  ...
  async run(message) {
    ...
    const server =
      (await Guilds.findById(message.guild.id)) ||
      (await Guilds.create({ _id: message.guild.id }));

    const user =
      (await Users.findById(message.author.id)) ||
      (await Users.create({ _id: message.author.id }));
      ...
      i18next.use(translationBackend).init(
        ...
        async (err, f) => {
          if (f) {
              ...
              message.mentions.users.forEach(async member => {
                ...
                if (user) {
                  if (user.afk) {
                    if (!user.afkReason) {
                      message.reply(
                        `\`${member.tag}\` ${t('commands:afk.no-reason')}`
                      );
                      return;
                    }

                    message.reply(
                      `\`${member.tag}\` ${t('commands:afk.with-reason', {
            ...
            let member = await this.client.database.Users.findById(
              message.author.id
            );
            ...
            if (message.content === message.guild.me.toString()) {
              message.channel.send(
                `${t('events:mention.start')} ${
                  message.author
                }, ${t('events:mention.end', { prefix: server.prefix })}`
              );
            }
            /*if (server.partner) {
            ...
            if (user.blacklist) {
              const embed = new RichEmbed()
                ...
                .setDescription(
                  `Olá ${message.author}, parece que você fez besteira que acabou quebrando os meus termos de uso, devido à isto, você foi banido de me usar.`
                )
                .addField('Motivo', user.blacklistReason)
                .addField(
                  'Banido injustamente?',
                  `Se você acha que foi banido injustamente, então entre em contato com a ${
                    this.client.users.get('395788326835322882').tag
                  } ou entre no meu servidor de suporte.`
                );
            ...
            if (server.commandNull === true) {
              if (!comando)
                return message.chinoReply('error', t('events:command-null'));
            } else {
              if (!comando) return;
            }
            ...
            if (comando.config.OnlyDevs) {
              if (!this.client.config.owners.includes(message.author.id))
                return message.chinoReply('error', t('permissions:ONLY_DEVS'));
            }
            if (cooldown.has(message.author.id)) {
              let time = cooldown.get(message.author.id);
              return message.chinoReply(
                'error',
                t('events:cooldown.message', {
                  time:
                    time - Date.now() > 1000
                      ? moment
                          .utc(time - Date.now())
                          .format(`ss [${t('events:cooldown.secounds')}]`)
                      : moment
                          .duration(time - Date.now())
                          .format(`[${t('events:cooldown.milliseconds')}]`)
                })
              );
            }
            ...
            if (userPermission !== null) {
              if (!message.member.hasPermission(userPermission)) {
                ...
                return message.chinoReply(
                  'error',
                  `${t('permissions:USER_MISSING_PERMISSION', { perm: perm })}`
                );
              }
            }
            if (clientPermission !== null) {
              if (!message.member.hasPermission(clientPermission)) {
                let perm = clientPermission
                  .map(value => t(`permissions:${value}`))
                  .join(', ');
                return message.chinoReply(
                  'error',
                  `${t('permissions:CLIENT_MISSING_PERMISSION', {
                    perm: perm
                  })}`
                );
              }
              ...
              new Promise((res, rej) => {
                ...
                .catch(err => {
                  ...
                  const embed = new RichEmbed()
                    ...
                    .setTitle(
                      `${this.client.emotes.chino_sad} ${t('events:error')} ${
                        this.client.emotes.chino_chibi
                      }`
                    )
                    .setDescription(`\`\`\`js\n${err.stack}\`\`\``);
