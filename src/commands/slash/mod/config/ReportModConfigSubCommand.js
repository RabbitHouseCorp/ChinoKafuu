  ...
  run(ctx) {
    switch (ctx.args.get('status').value) {
      case status.TYPE_1: {
        ...
        ctx.replyT('success', 'commands:config.select.channeLSelectedWithSuccess')
        return
      }
      case status.TYPE_2: {
        // Tag:  CHANNEL_SELECTED_WITH_SUCCESS
        // Message: The module is already disabled!
        if (ctx.db.guild.reportModule === false) return ctx.replyT('error', 'commands:config.channel.moduleHasDisabled')
        ...
        ctx.replyT('success', 'commands:config.select.disableDescription')
        return
      }
    }
    ...
    ctx.replyT('error', 'commands:config.channel.moduleIfFoundBug')
    new Error({})
  }
}
