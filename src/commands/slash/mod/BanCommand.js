...
module.exports = class BanCommand extends Command {
  ...
  async run(ctx) {
    ...
    const reason = ctx._locale('basic:punishment.reason', {
      0: `${ctx.message.member.user.username}#${ctx.message.member.user.discriminator}`, 1: ctx.message.command.interface.get('reason') ?
        ctx.message.command.interface.get('reason').value : ctx._locale('basic:noReason')
    })
    let quantity = ctx.message.command.interface.get('message_days')?.value
    ...
    try {
      ctx.client.banGuildMember(ctx.message.guild.id, member.id, quantity, reason).then(() => {
        ...
        embed.addField(ctx._locale('basic:punishment.embed.reason'), ctx.message.command.interface.get('reason').value)
