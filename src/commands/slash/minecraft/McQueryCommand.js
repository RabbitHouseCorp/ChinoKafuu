...
module.exports = class McQueryCommand extends Command {
  ...
  async run(ctx) {
    const body = await axios.get(`${encodeURI(`https://api.mcsrvstat.us/2/${ctx.interactionMessage.command.interface.get('minecraft-server-ip').value}`)}`, { responseType: 'json' })
