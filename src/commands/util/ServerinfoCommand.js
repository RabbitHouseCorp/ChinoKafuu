...
module.exports = class ServerinfoCommand extends Command {
	...
	run({ message, args, server }, t) {
		...
		message.guild.icon ? embed.setAuthor(t("commands:serverinfo.name", { name: message.guild.name }), message.guild.iconURL({ format: "png", dynamic: true })) : null
		embed.setColor(this.client.colors.default)
		message.guild.icon ? embed.setThumbnail(`${message.guild.iconURL({ format: "png", dynamic: true })}`) : null
		...
		page2.setAuthor(t("commands:serverinfo.name", { name: message.guild.name }), message.guild.iconURL({ format: "png", dynamic: true }))
		page2.setColor(this.client.colors.default)
		message.guild.icon ? page2.setThumbnail(`${message.guild.iconURL({ format: "png", dynamic: true })}`) : null
		page2.addField(t("commands:serverinfo.guildChannel", { totalChannel: message.guild.channels.cache.size }), t("commands:serverinfo.channels", { voice: voice, text: text, category: category }), true)
